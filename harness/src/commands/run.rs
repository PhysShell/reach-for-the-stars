...
async fn run_locked(
    ...
    let (pointer_bytes, pointer_etag) = primary.get(LATEST_KEY).await.context("get latest.json")?;
    let prev_pointer: LatestPointer =
        serde_json::from_slice(&pointer_bytes).context("parse latest.json")?;
    tracing::info!(version = %prev_pointer.version, "pointer loaded");

    let (ciphertext, _) = primary
        .get(&prev_pointer.object)
        .await
        .with_context(|| format!("get snapshot {}", prev_pointer.object))?;
    verify_digest(&prev_pointer.object_sha256, &ciphertext)?;

    let (sig_bytes, _) = primary
        .get(&prev_pointer.signature_object)
        .await
        .with_context(|| format!("get signature {}", prev_pointer.signature_object))?;
    ...
    .await?;
    session.import_storage_state(&state).await?;
    ...
    if !canary_result.ok {
        ...
        bail!(
            "canary failed: status={}, observed={:?}; refusing to overwrite snapshot",
            canary_result.status,
            canary_result.observed
        );
    }
    ...
    primary
        .put_if_unmodified(LATEST_KEY, new_pointer_bytes.clone(), Some(&pointer_etag))
        .await
